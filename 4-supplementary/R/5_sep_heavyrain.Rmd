...
df_washb_gps_average_daily_ppts <- df_washb_gps_average_daily_ppts %>%
  ...
  monsoonf = ifelse(svyyear.y == "2014" & #svyyear.y comprises of 2013, 2014 and 2015
            ...
            ifelse(svyyear.y == "2015" &
                   svyweek.y >= 14 &
                   svyweek.y <= 39, 1, 0)),
  monsoon.f = factor(monsoonf, levels = c(0,1), labels = c("dry","monsoon")),
  heavy_rain_indf = factor(heavy_rain_ind, levels = c("FALSE","TRUE"), labels = c("No heavy rain","Heavy rain"))
  )

table(df_washb_gps_average_daily_ppts$svyyear.y, df_washb_gps_average_daily_ppts$svyweek.y)
table(df_washb_gps_average_daily_ppts$monsoon.f)
...
df_washb_gps_average_daily_ppts <- df_washb_gps_average_daily_ppts %>%
  mutate(diar7d = case_when(
    ...
    heavy_rain_indf = case_when(
      heavy_rain_indf == "No heavy rain" ~ 0,
      heavy_rain_indf == "Heavy rain" ~ 1,
      TRUE ~ as.numeric(NA)
    ))
...
ggsave(plot_prev_composite, file = here::here("4-supplementary", "output",
                      "plot_effectmodsepmonsoon_glm.pdf"), height=6,width=4)
