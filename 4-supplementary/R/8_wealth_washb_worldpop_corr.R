...
washbgps_df <- left_join(df_gps_publicid_washb_sepseason, 
               ...
               select(clusterid,wealthscore,mean_wealth,mean_wealthrank,wealth_tertile,geometry) #%>%
               #mutate(wealth_rank_washb = rank(wealthscore)/nrow(.),
               #wealth_rank_washb = round(wealth_rank_washb,2))
...
wealth_join <- st_join(washbgps_df_sf, wealth_popunder3_gps_sf) %>%
  select(clusterid,wealthscore,mean_wealth,mean_wealthrank,wealth_tertile,
...
# correlation scatter plot 
wealth_corr <- ggscatter(wealth_join,
  ...
  stat_cor(aes(label = paste(..r.label.., ..p.label.., sep = "~`,`~")),
           ...
           size = 5, 
           label.x.npc = 0.5, label.y.npc = 0.80 
  ) 
...
ggsave(wealth_corr, file = here::here("4-supplementary", "output",
