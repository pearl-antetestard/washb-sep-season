...
df_analysis_svy12 <- df_analysis_svy12 %>%
  ...
  edu_rank = rank(momeduy)/nrow(.), # range of maternal education (0-16)
  block=factor(block),
...
mod_gam_1 = gam(diar7d ~ Arms + s(momeduy, bs = "cr", by = Arms) +
...
mod_gam_2 = gam(diar7d ~ Arms + s(momeduy, bs = "cr", by = Arms) +
...
# smooth difference
gam_diff <- get_smooths_difference(mod_gam_1, momeduy, difference = list(Arms = c("Control",
                                      "Intervention")))
print(gam_diff)

# smooth ratio - the difference would need to be exponentiated
gam_ratio <- get_smooths_difference(mod_gam_2, momeduy, difference = list(Arms = c("Control",
                                      "Intervention")))
print(gam_ratio)
...
# Diarrhea prevalence
gam_plot_prev <- plot_smooths(model = mod_gam_1, series = momeduy, 
                         comparison = Arms) +
                 ...
                 scale_y_continuous(breaks=seq(-0.04,0.14,by=0.02), limits = c(-0.04,0.14),
                                   labels =  sprintf("%1.0f",seq(-0.04,0.14,by=0.02)*100)) +
                 labs(colour = "Arms", fill = "Arms", linetype = "Arms") +
                 labs(x = "Maternal education (years)",
...
plot_ratio <- ggplot(gam_ratio, aes(momeduy, exp(difference)#, group=group
                     )) +
              ...
              scale_x_continuous(breaks=seq(0,16,by=2)) +
              #scale_y_continuous(breaks=c(-1,-0.5,1,1.5,2,4),trans = "log") +
              labs(x = "Maternal education (years)",
...
plot_diff <- ggplot(gam_diff, aes(momeduy, difference#, group=group
                                  )) +
...
ggsave(plot_prev_composite, file = here::here("4-supplementary", "output",
                                           "plot_effectmod_sep_gam_eduyears.png"), height=6.5,width=4)
```
