...
```{r baseline indicators by treatment}
...
table1(~ agedays + Sex + `Wealth tertiles` + wealthscore + `Mother's education` + 
         ...
         `Improved roof material (tin, cement)` + `Improved wall material (wood, brick, thin)` + 
         `Improved floor (wood, concrete)` + `Household has electricity` + `Household has radio` + 
         `Household has refrigerator` + `Household has bicycle` + `Household has motorcycle` + 
         `Household has sewing machine` + `Number of land phone` + `Has BW or color TV` + 
         `Has >=1 wardrobe` + `Has >=1 table`+ `Has >=1 chair` + `Has >=1 clock` + `Has >=1 khat` + 
         `Has >=1 chouki` + `Has >=1 mobile`
            | tx_table1, data = ., 
...
```{r baseline indicators by treatment and SEP}
...
table1(~ agedays + Sex + `Mother's education` + 
         Nhh + Ncomp + Diarrhea + `Primary water source` + `Store drinking water` + 
         `Reported treating water today/tomorrow` + `Own their latrine` + `Latrine has concrete slab` + 
         `Latrine has functional water seal` + `No visible feces on floor of latrine` + 
         `Has a potty for defecation` + `Primary handwashing location has water/soap` + 
         `Improved roof material (tin, cement)` + `Improved wall material (wood, brick, thin)` + 
         ...
         `Has >=1 wardrobe` + `Has >=1 table`+ `Has >=1 chair` + `Has >=1 clock` + `Has >=1 khat` + 
         `Has >=1 chouki` + `Has >=1 mobile` | 
         tx_table1*`Wealth tertiles`, data = ., 
         render = rndr, topclass="Rtable1-grid Rtable1-left"
         ) 
...
# Baseline characteristics by intervention and monsoon season

```{r baseline indicators by treatment and monsoon}
