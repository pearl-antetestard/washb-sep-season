...
# Diarrhea prevalence
gam_plot_prev_dry <- plot_smooths(model = mod_gam_1, series = wealth_rank, 
                         comparison = Arms) +
                 ...
                 labs(x = "Wealth rank based on the wealth index score",
                      y = "Diarrhea Prevalence (%) \nduring dry season", title = "B. Dry season"
                      ) +
                    #title = "gam") +
...
plot_ratio_dry <- ggplot(gam_ratio, aes(wealth_rank, exp(difference)#, group=group
                     )) +
              ...
              scale_y_continuous(breaks=c(0.5,0.8,1,1.5,2,3,4), limits = c(0.5,4),
                                 trans = "log") +
              labs(x = "Wealth rank based on the wealth index score",
              y = "Prevalence Ratio (PR) \nduring dry season", title = " "
              ) +
...
plot_diff_dry <- ggplot(gam_diff, aes(wealth_rank, difference#, group=group
                                  )) +
            ...
            scale_y_continuous(breaks=seq(-0.04,0.10,by=0.02), limits = c(-0.04,0.10),
...
plot_prev_composite_dry <- plot_grid(gam_plot_prev_dry, plot_ratio_dry, plot_diff_dry, 
                               nrow=3, align = "h") 
plot_prev_composite_dry

```

```{r merge rainy and dry seasons}


plot_prev_composite_rainy_dry <- plot_grid(plot_prev_composite_rainy, plot_prev_composite_dry, 
                              ncol = 2, nrow = 1, align = "hv")
plot_prev_composite_rainy_dry


ggsave(plot_prev_composite_rainy_dry, file = here::here("2-initial-analysis", "output",
                                           "plot_effectmod_gam_rainy_dry.png"), height=6.5,width=6.5)
...
ggsave(plot_prev_composite, file = here::here("2-initial-analysis", "output",
                      "plot_effectmodsep_gam_dry.png"), height=8,width=5)
